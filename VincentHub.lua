...
local Window = ArrayField:CreateWindow({
    ...
    LoadingSubtitle = "by vincent and allzkie",
    ConfigurationSaving = {
        Enabled = true,
        FolderName = nil,
    ...
    KeySettings = {
        Title = "VINCENT HUB ",
        Subtitle = "Key System",
        Note = "Join our discord (https://discord.gg/pj5vc6mg)",
        FileName = "VINCENT HUB BY  Key System",
        SaveKey = True,
...
local Dropdown = Tab3:AddDropdown({
    ...
    Callback = function(currentOption)
        if autoParryAIEnabled then
            ...
            local Notify = Library:MakeNotify({
                Title = "VINCENT HUB ",
                Text = "VINCENT HUB  cannot activate mode " .. currentOption .. " since you do not have Auto Parry [AI] mode activated.",
                Time = 5
              })
        end
    end
...
local function createNotification(player, deviceType)
    local Notify = Library:MakeNotify({
        Title = "VINCENT HUB  ",
        Text = "VINCENT HUB  detected a rapid click on " .. player.Name .. "'s " .. deviceType,
        Time = 5
    })
end
